...
def test_read_file(db_connector):
    db_connector._DBConnector__file_json = path.join(ROOT_DIR, "tests", "test.json")
    with open(db_connector._DBConnector__file_json, "w", encoding="UTF-8") as file:
        file.write("")
    assert db_connector.read() == []
    remove(db_connector._DBConnector__file_json)
